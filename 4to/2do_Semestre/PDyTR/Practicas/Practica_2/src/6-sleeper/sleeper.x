...
program SLEEPER_PROG {
   version SLEEPER_VERSION {
     int SLEEP_TIME(sleeping_time) = 1;
   } = VERSION_NUMBER;
} = 555555555;


/* This defines the RPC program number as 555555555, although within the
client and server code I can just refer to it as SLEEPER_PROG. In other words,
rpcgen will put this line in the .h file:

#define SLEEPER_PROG 555555555

The version number is 1, but I can use SLEEPER_VERSION.
...
------------------------------
On the client side, a stub will be generated in sleeper_clnt.c for each
of the remote procedures. The stub prototype for the add procedure looks like
this:

int * sleep_time_1(sleeping_time *, CLIENT *);
...
On the server side, rpcgen creates the file sleeper_svc.h that includes
...
int *sleep_time_1_svc(sleeping_time*, struct svc_req *);
